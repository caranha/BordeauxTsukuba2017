...
class MessageRenderer implements Observer {
    ...
    private Vector2 onScreenPosition, onWorldPosition;
    private Vector2 startingPosition; // on camera
    ...
    public void update(Observable o, Object arg) {
        ...
        if (!content.equals("")) {
            ...
            targetWidth = getTargetWidth(message.getLength(), isHorizontal(direction));
            ...
            // Start tweens
            if (message.getTimeToAnswer() != 0.f) {
                alphaTween.playWith(1, 0, message.getTimeToAnswer(), message.getTimeToUnderstand(), true);
            }
    ...
    private float getTargetWidth(int length, boolean horizontal) {
        float scale = horizontal ? 0.4f : 1.f;
        return Math.min(400.f * scale, length * 30.f * scale);
    }

    private void initScreenPosition(float x, float y) {
        onWorldPosition = new Vector2(x * TILE_SIZE, y * TILE_SIZE);
    ...
    void render(Batch worldBatch) {
        if (updatePosition()) {
            ...
            // set alpha
            Color color = screenBatch.getColor();
            screenBatch.setColor(color.r, color.b, color.b,
                    alphaTween.isPlaying() ? alphaTween.getInterpolation() : 1);
    ...
    private void drawBackground() {
        ...
        screenBatch.draw(background,
                onScreenPosition.x - padding + (1 - interpolation) * (padding + layout.width / 2),
                onScreenPosition.y - layout.height - padding + (1 - interpolation) * (padding + layout.height / 2),
                (layout.width + padding * 2) * interpolation,
                (layout.height + padding * 2) * interpolation);
    }
    ...
    private boolean updatePosition() {
        if (message != null && content != null &&
                onScreenPosition != null && onWorldPosition != null && !content.equals("")) {
            onScreenPosition = toScreenPosition(startingPosition.x, startingPosition.y);
            execute(offsetCalculator, direction);
            onWorldPosition = toWorldPosition(onScreenPosition.x, onScreenPosition.y);
            return true;
        }
        return false;
    }
    ...
    private Vector2 toWorldPosition(float x, float y) {
    ...
    private void initOffsetCalculations() {
        ...
        offsetCalculator.put(EAST, () -> onScreenPosition.add(
                -Math.min(targetWidth, layout.width) - padding * 5, layout.height / 2));
